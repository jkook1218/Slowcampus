...
#FACTORS = [Date,Open,High,Low,Close,Adj Close,Volume]
FACTORS = ['Open', 'Close']

INPUT_DIM = len(FACTORS)
HIDDEN_NODES = 10
PREDICTSIZE = 1  # 3초간의 속도, RPM을 예측.
...
def prepare_data_simple(df):
  ...
  print('X', x.shape)
  print('Y', y.shape)
  training_x, validation_x, test_x = LSTM.split_data(x, val_size=0.1, test_size=0.1)
  training_y, validation_y, test_y = LSTM.split_data(y, val_size=0.1, test_size=0.1)
...
def do_train_rnn(training_x, training_y, validation_x, validation_y, name, plot=False, fname=None):
  ...
  lstm = LSTM.LSTM(INPUT_DIM, WINDOWSIZE, HIDDEN_NODES, output_dim, PREDICTSIZE, name='NEXT3')
  lstm.set_name('lstm_%s_%s.net' % (fname, name))
  lstm.set_validation_data(validation_x, validation_y, valid_stop=0.0002)
  
  nn_loaded = lstm.load()
  if not nn_loaded:
    lstm.set_training_stop(0.005)
